---
title: "Wind Speed: Data Carpentry"
...
We can check if there were _any_ errors by `map()`ping `raw_dfs` and grabbing the `$error` portion. `compact()` will remove any `NULL` values, and `length()` will tell us how long the list of final results is.
